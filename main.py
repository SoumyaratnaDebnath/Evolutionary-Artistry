...
if uploaded_file is not None:
    ...
    with st.container():
        st.markdown("<h3 style='text-align: center;'>Visualizing the image with a Spanning Tree optimized using Genetic Algorithm <br>and using Game of Life to create artworks.</h3>", unsafe_allow_html=True)
        st.write('---')
        with st.container():
            ...
            with col2:
                ...
                with col1:
                    num_generations_ga = st.slider("##### Generations in Genetic Algoritm", 500, 2500, 2000, 500)
                    num_generations_gol = st.slider("##### Generations in Game of Life", 100, 5000, 500, 100)
                with col3:
                    solution_per_population = st.slider("##### Population Size", 10, 100, 50, 10)
                    image_size = st.slider("##### Image Size", 128, 512, 512, 2)
                    save_frequency = 100
